...
2026-03-19: Khôi phục ngữ cảnh bead `cng-h1d` từ `task.md`, `task_plan.md`, `findings.md`, `progress.md`, và bead tracker; xác nhận phần còn lại là chốt permission matrix + verify.
2026-03-19: Dọn lỗi biên dịch còn sót ở `CustomersPage.tsx` (import React trùng) và căn chỉnh lại `role-permissions-manager.test.tsx` theo contract thực của `select` dùng `role.id`.
2026-03-19: Verify targeted frontend pass `16/16` (`auth-guards`, `customers-page.permissions`, `imports-page.fixed-type`, `role-permissions-manager`, `admin-users-page`); backend route test `AdminEndpointsRouteTests` pass `1/1`.
2026-03-19: Đồng bộ notebook hoàn tất và đóng bead `cng-h1d` trong `bd`.
