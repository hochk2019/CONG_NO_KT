...
invoice_reduction_alloc AS (
    ...
    FROM congno.invoice_reduction_applications ira
    JOIN congno.invoices ri ON ri.id = ira.reduction_invoice_id
    WHERE ri.deleted_at IS NULL AND ri.status <> 'VOID'
    GROUP BY ira.applied_invoice_id
),
advance_alloc AS (
    SELECT ra.advance_id, SUM(ra.amount) AS allocated
...
combined AS (
    SELECT outstanding FROM invoice_out WHERE outstanding > 0 AND due_date <= CURRENT_DATE
    UNION ALL
    SELECT outstanding FROM advance_out WHERE outstanding > 0 AND due_date <= CURRENT_DATE
),
collected_30d AS (
    SELECT COALESCE(SUM(r.amount), 0) AS total
    FROM congno.receipts r
...
SELECT COALESCE((SELECT SUM(outstanding) FROM combined), 0) + COALESCE((SELECT total FROM collected_30d), 0) AS total;
";
