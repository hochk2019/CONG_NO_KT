...
CREATE TABLE IF NOT EXISTS import_batches (
  ...
  status varchar(16) NOT NULL DEFAULT 'STAGING', -- STAGING/COMMITTED/ROLLED_BACK/CANCELLED
  summary_data jsonb,
  ...
  CONSTRAINT ck_import_status CHECK (status IN ('STAGING','COMMITTED','ROLLED_BACK','CANCELLED'))
);
...
CREATE INDEX IF NOT EXISTS ix_import_batches_status_created_at ON import_batches(status, created_at DESC);
...
CREATE INDEX IF NOT EXISTS ix_import_staging_batch_created ON import_staging_rows(batch_id, created_at DESC);
...
ON invoices(customer_tax_code, invoice_no) WHERE deleted_at IS NULL;
...
ON advances(customer_tax_code, advance_no) WHERE deleted_at IS NULL;
...
CREATE TABLE IF NOT EXISTS receipts (
  ...
  applied_period_start date, -- first day of month (e.g. 2025-12-01)
    ...
    allocation_status varchar(16) NOT NULL DEFAULT 'UNALLOCATED', -- UNALLOCATED/SELECTED/SUGGESTED/ALLOCATED/PARTIAL/VOID
    allocation_priority varchar(16) NOT NULL DEFAULT 'ISSUE_DATE', -- ISSUE_DATE/DUE_DATE
    allocation_targets jsonb,
    allocation_source varchar(16), -- MANUAL/AUTO
    allocation_suggested_at timestamptz,
    ...
    CONSTRAINT ck_receipt_alloc_status CHECK (allocation_status IN ('UNALLOCATED','SELECTED','SUGGESTED','ALLOCATED','PARTIAL','VOID')),
    CONSTRAINT ck_receipt_alloc_priority CHECK (allocation_priority IN ('ISSUE_DATE','DUE_DATE')),
...
ON receipts(customer_tax_code, receipt_no) WHERE deleted_at IS NULL;
...
-- Seed sellers (optional)
INSERT INTO sellers(seller_tax_code, name, address) VALUES
('2300328765', 'CÔNG TY TNHH TIẾP VẬN HOÀNG KIM', NULL),
('2301098313', 'CÔNG TY TNHH GIẢI PHÁP ĐẦU TƯ HOÀNG MINH',
 'Phòng 405 Tòa nhà Trung Thành số 10 Nguyễn Đăng Đạo, Phường Tiền Ninh Vệ, TP Bắc Ninh, Việt Nam')
